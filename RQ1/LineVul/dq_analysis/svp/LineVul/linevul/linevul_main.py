...
import wandb
wandb.init(project='case-study-linevul')
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    # Initialize tracking variables and start training
    global_step = 0
    tr_loss = 0.0
    best_f1 = 0
    model.zero_grad()
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            outputs = model(input_ids=inputs_ids, labels=labels, output_attentions=True)
            loss = outputs.loss
            ...
            tr_loss += loss.item()
            bar.set_description(f"epoch {idx} loss {tr_loss / (step + 1)}")
            ...
            # Optionally, save attentions per epoch or batch
            all_attention_weights.append(attentions)

        # Evaluate per epoch
        results = evaluate(args, model, tokenizer, eval_dataset, eval_when_training=True)    

    # After training is complete, save the attention weights
    attention_weights_path = os.path.join(args.output_dir, 'attention_weights.pkl')
    with open(attention_weights_path, 'wb') as f:
        pickle.dump(all_attention_weights, f)
    print(f"Attention weights saved to {attention_weights_path}")
            ...
            for mini_batch in progress_bar:
                ...
                if result_df["logits"][index] > 0.5:
                    ...
                    line_level_localization(flaw_lines=result_df["flaw_line"][index],
                                            ...
                                            original_func=result_df["Function"][index], 
                                            args=args,
                                            top_k_loc=None,
                                            top_k_constant=None,
                ...
                if index in tp_indices:
                    ...
                    if isinstance(df["flaw_line"][index], str) and isinstance(df["flaw_line_index"][index], str):                        
                        ...
                        line_level_localization_tp(flaw_lines=df["flaw_line"][index],
                                                ...
                                                model=model, 
                                                mini_batch=mini_batch, 
                                                original_func=df["Function"][index], 
                                                args=args,
...
def generate_result_df(logits, y_trues, y_preds, args):
    df = pd.read_csv(args.test_data_file)
    all_num_lines = []
    all_processed_func = df["Function"].tolist()
    ...
    return pd.DataFrame({"logits": logits, "y_trues": y_trues, "y_preds": y_preds, 
                         ...
                         "flaw_line": df["flaw_line"], "Function": df["Function"]})
