import sys
import os
sys.path.append(os.getcwd() + '/dq_analysis')
import time
import json
import pandas as pd
from datasets.data import Data
from svp.run import run_sota, get_random_string
from attributes.uniqueness import get_duplicate_clusters
from attributes.consistency import get_consistent_dataset, get_inconsistent_clusters
...
def benchmark(dataset, model):
    logging.info('Initializing: %s', dataset)
    tstart = time.time()
    ...
    logging.info('Dataset size - Train: %s, Validation: %s, Test: %s', len(train), len(val), len(test))
    logging.info('Initialization time: %s', time.time() - tstart)

    logging.info('Running: ')
    tstart = time.time()
    run_sota(train, val, test, model, dataset, 'benchmark', save=True)
    logging.info('Execution time: %s', time.time() - tstart)
...
def consistency(dataset, model, mode='bench'):
    ...
    if mode == 'bench':
        ...
        clusters = get_inconsistent_clusters(dataset, return_clusters=True)
        cross_inconsistent = []
...
def imbalance(dataset, model):
    """
    Investigate the impact of class imbalance using SMOTE to balance the dataset.
    Compare the model performance before and after applying SMOTE.
    """
    ...
    print(f'Original class distribution in training data: {original_dist}')

    # Apply SMOTE
    smote = SMOTE(random_state=42)
    X_train_smote, y_train_smote = smote.fit_resample(X_train, y_train)
    smote_dist = y_train_smote.value_counts()
    print(f'Class distribution after SMOTE in training data: {smote_dist}')

    # Combine resampled features and target into DataFrame for model input
    train_smote = pd.concat([X_train_smote, y_train_smote], axis=1)
    val = pd.concat([X_val, y_val], axis=1)
    test = pd.concat([X_test, y_test], axis=1)

    # Run the model with original data
    run_sota(X_train, X_val, X_test, model, dataset, 'original_data', save=True)

    # Run the model with SMOTE-applied data
    run_sota(X_train_smote, X_val, X_test, model, dataset, 'smote_data', save=True)

    print('Execution times: ')
    print(f'Original data: {time.time() - tstart} seconds')
    tstart = time.time()
    print(f'SMOTE data: {time.time() - tstart} seconds\n#####')

if __name__ == '__main__':

    dataset = sys.argv[1]
    benchmark(dataset, 'linevul')

    # For Concept Drift
    currency(dataset, 'linevul')

    # For Label Noise
    accuracy(dataset, 'linevul')
    consistency(dataset, 'linevul', 'train')

    # For Class Imbalance
    imbalance(dataset, 'linevul')
