...
data = pickle.load(open(f'./data/openstack/original/deepjit/openstack_test.pkl', 'rb'))
all_msg, all_msg_mask, all_predict, all_label = data

dictionary = pickle.load(open('data/{}/deepjit/{}_dict.pkl'.format(project, project), 'rb'))  
dict_msg, dict_code = dictionary
id2msg_world = get_world_dict(dict_msg)
id = uuid.uuid4()
for msg, maks, pred, label in zip(all_msg, all_msg_mask, all_predict, all_label):
    ...
    if 'task-number' in msg or 'fix' in msg or 'bug' in msg or 'failures' in msg \
        ...
        if label == 1 and pred > 0.5:
            visualize(uuid, pred, label, maks, msg, msg=True)