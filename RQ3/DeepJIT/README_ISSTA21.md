...
Complete Evaluation: In addition to the general requirements of Retraining Evaluation (extra 200GB free disk space in this case), MongoDB is required for the data extraction process. You can install MongoDB by the following tutorial: mongodb-4.4.4.
...
Docker Container
To simplify the testing process of the artifacts, we provide a docker container which can be downloaded from an online docker hub. Please note that the size of our docker container including the dataset is about 30GB, so please prepare sufficient disk space. And to using GPU in docker, you should install docker with version >= 19.04 and install NVIDIA Container Runtime. Then, you can install and run the docker files by following commands below.
...
We provide data after pre-processing from our prior runs to faciliate model training for artifact evaluation. Users can download it from an online Cloud Drive to avoid time-consuming data extraction process. Specifically, you need to download and unzip the datasets to the target directory.
...
Please note that the train_cc2vec process may take about 3 days for -RQ2_T5 and -RQ4_T8, 1 day for other $RQs! The pred_cc2vec process can take 1 day for -RQ2_T5 and -RQ4_T8, 3 hours for other $RQs. The train_deepjit process may take about 6 hours to 1 day for different $RQs and the pred_deepjit process can take about 10 minutes. At last, the trained models and predicted results are stored in the CC2Vec/snapshot/$project folder.
