...
def mini_batches_smote(X_msg, X_code, Y, mini_batch_size=64, seed=0):
    np.random.seed(seed)
    random.seed(seed)

    # Combine X_msg and X_code for SMOTE
    # Assuming X_code can be reshaped into two dimensions; adjust if the structure is different
    X_code_flat = X_code.reshape(X_code.shape[0], -1)
    X_combined = np.hstack((X_msg, X_code_flat))

    # Apply SMOTE
    smote = SMOTE(random_state=seed)
    X_res, Y_res = smote.fit_resample(X_combined, Y)

    # Split X_res back into X_msg and X_code
    X_msg_res = X_res[:, :X_msg.shape[1]]
    X_code_res = X_res[:, X_msg.shape[1]:].reshape(-1, X_code.shape[1], X_code.shape[2])

    m = X_msg_res.shape[0]  # Updated number of samples
    mini_batches = []
    num_complete_minibatches = m // mini_batch_size

    # Shuffle the dataset
    permutation = list(np.random.permutation(m))
    shuffled_X_msg = X_msg_res[permutation]
    shuffled_X_code = X_code_res[permutation]
    shuffled_Y = Y_res[permutation]

    for k in range(num_complete_minibatches):
        mini_batch_X_msg = shuffled_X_msg[k * mini_batch_size:(k + 1) * mini_batch_size]
        mini_batch_X_code = shuffled_X_code[k * mini_batch_size:(k + 1) * mini_batch_size]
        mini_batch_Y = shuffled_Y[k * mini_batch_size:(k + 1) * mini_batch_size]
        mini_batches.append((mini_batch_X_msg, mini_batch_X_code, mini_batch_Y))

    # Handling the end case (last mini-batch < mini_batch_size)
    if m % mini_batch_size != 0:
        mini_batch_X_msg = shuffled_X_msg[num_complete_minibatches * mini_batch_size:]
        mini_batch_X_code = shuffled_X_code[num_complete_minibatches * mini_batch_size:]
